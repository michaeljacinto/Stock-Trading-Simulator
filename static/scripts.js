...
// SOURCE = https://widgets.freestockcharts.com/ScrollingTicker.aspx?type=gainers
// All rights reserved to their respective owners
if(!window.wbiGlobalScope){
    window.wbiGlobalScope=[]
}function ieIntervalHandler(id,strFunc){
    var scope=window.wbiGlobalScope[id];eval("scope."+strFunc+"()")
}function WBIHorizontalTicker(a){
    this.textW=0;this.containerW=0;
    this.scroller=null;
    this.destLoc=0;this.moveAmount=1;
    this.moveInterval=10;
    this.refreshData=20000;
    this.pauseAnimation=true;this.sub_symbols;
    this.updatingList=false;this.scrolling=false;
    this.animationStarted=false;this.listName=a;
    this.scollerID="";this.timer=null;
    this.alsoMove=null;this.animate=true;
    this.animationHost=null;this.start=function(){
        var c=new Date();this.scollerID="wbi_scroller_"+c.getSeconds().toString()+c.getMilliseconds().toString()+Math.floor(Math.random()*101).toString();
        var b='<div class="wbiscroller"><div class="scrollingtext" id="'+this.scollerID+'"></div></div>';
        document.writeln(b);window.wbiGlobalScope[this.scollerID.toString()]=this;
        var d="#"+this.scollerID;this.scroller=$(d);
        this.scroller.mouseenter(this.OnMouseOver);
        this.scroller.mouseleave(this.OnMouseOut);
        this.UpdateList();this.startTimer()};
    this.startTimer=function(){if(document.all){
        setInterval('ieIntervalHandler("'+this.scollerID+'","UpdateValues")',this.refreshData)
    }else{
        this.timer=setInterval(function(b){b.UpdateValues()},this.refreshData,this)}};
    this.StartAnimationTimer=function(){
        if(document.all){setInterval('ieIntervalHandler("'+this.scollerID+'","AnimateLoop")',this.moveInterval)
        }else{this.timer=setInterval(function(b){b.AnimateLoop()},this.moveInterval,this)}};
    ...
    this.OnMouseOver=function(){var b=$(this).attr("id");
    ...
    this.configureAnimation=function(){
        this.textW=this.scroller.width();this.containerW=this.scroller.parent().width();
        this.setInitialLocation();this.destLoc=-this.textW+(this.containerW*0.15);
        if(this.animate){
